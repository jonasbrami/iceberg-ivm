...
async def test_concurrent_triggers_for_same_view_do_not_overlap(setup_state):
    """Repro for #23 / #24.

    With a per-view worker as the sole caller of ``refresh_view``, concurrent
    triggers (manual POSTs, interval ticks, …) coalesce into sequential
    refreshes. No two ``refresh_view`` bodies are ever in flight at once.
    """
    from trino_mv_orchestrator import server as server_mod
