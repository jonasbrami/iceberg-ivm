...
async def query_bars(cursor):
    await cursor.execute(
        f"SELECT symbol, minute, open, high, low, close, volume, trade_count FROM {TARGET_TABLE} ORDER BY symbol, minute"
    )
    return [
        dict(zip(["symbol", "minute", "open", "high", "low", "close", "volume", "trade_count"], r, strict=False))
