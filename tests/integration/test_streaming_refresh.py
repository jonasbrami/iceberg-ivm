...
async def _run_scenario(trino_conn, app_state, scenario: list[Cycle]) -> None:
    ...
    for i, cycle in enumerate(scenario, start=1):
        ...
        assert vs.last_action == cycle.expect_action, (
            f"{label}: expected last_action={cycle.expect_action!r}, "
            f"got {vs.last_action!r}"
        )
        ...
        assert vs.total_refreshes == expected_total_refreshes, (
            f"{label}: total_refreshes={vs.total_refreshes}, "
            f"expected {expected_total_refreshes}"
        )
...
class TestSustainedStream:
    ...
    async def test_ten_cycles(self, trino_conn, app_state):
        scenario = [
            ...
            ], expect_action="full", note="initial 3 trades, target created"),
            Cycle(rows=[
...
class TestMultiSymbol:
    ...
    async def test_three_symbols_eight_cycles(self, trino_conn, app_state):
        scenario = [
            ...
            ], expect_action="full", note="all three symbols open"),
            Cycle(rows=[
                Trade("AAPL", _ts("2026-04-08", "09:30:30"), 151.0, 200),
            ], note="AAPL only"),
            Cycle(rows=[
    ...
    async def test_optimize_between_appends(self, trino_conn, app_state):
        scenario = [
            Cycle(rows=[
                Trade("AAPL", _ts("2026-04-08", "09:30:00"), 150.0, 100),
                Trade("AAPL", _ts("2026-04-08", "09:30:30"), 151.0, 200),
            ], expect_action="full", note="initial appends"),
            Cycle(rows=[
        ...
        for i, cycle in enumerate(scenario, start=1):
            ...
            assert vs.last_action == cycle.expect_action, (
                f"{label}: last_action={vs.last_action!r}, "
                f"expected {cycle.expect_action!r}"
            )
            ...
            if cycle.compact:
                ...
                assert actual == prev_target_rows, (
                ...
                assert cur_snap is not None and (
                    ...
                    f"{label}: snapshot bookmark did not advance "
                    f"(prev={prev_snapshot}, cur={cur_snap})"
                )
